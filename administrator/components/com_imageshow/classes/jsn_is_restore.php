 ...
 * @version $Id: jsn_is_restore.php 12370 2012-04-27 09:53:25Z giangnd $
...
class JSNISRestore
{
	...
	function restore($config)
	{
		...
		{
			$this->_error 	 = true;
			$this->_msgError = JText::_('RESTORE_EXTRACT_FACKAGE_FAILURE');
		}
	...
	*/
	...
	/*function _migrateThemeProfileFromVersion3()
	{
		$queries 		= array();
		$checkShowCase 	=& $this->_document->getElementByPath('showcases');
				...
				if (count($attributes))
				{
					...
					foreach ($attributes as $attribute)
					{
						...
						foreach ($attribute as $key => $value)
						{
							if ($this->_checkTableColumExist('#__imageshow_showcase', $key))
							{
								if($key == 'showcase_id' || $key == 'theme_id')
								{
									$fields 	 .= $key.',';
									$fieldsValue .= $this->_db->quote($value).',';
								}
						...
						$queries [] = 'INSERT #__imageshow_theme_profile ('.substr($fields, 0, -1).', theme_name) VALUES ('.substr($fieldsValue, 0, -1).', '.$this->_db->quote('themeclassic').')';
					}
				}
			}
		}
					...
					foreach ($attributes as $attribute)
					{
						...
						foreach ($attribute as $key => $value)
						{
							...
							if (($key == ('general_round_corner_radius' || 'general_border_color'
								...
								&& $this->_checkTableColumExist('#__imageshow_theme_classic', $key))
							{
								$fields[] = $key.'='.$this->_db->quote($value);
							}

							if ($key == 'theme_id' && $this->_checkTableColumExist('#__imageshow_theme_classic', $key)) {
								$themeID = $value;
							}
						}

						$queries [] = 'UPDATE #__imageshow_theme_classic SET '.implode(',', $fields).' WHERE theme_id = '. $themeID ;
					}
				}
			}
		}
